...
class MultiExpertMultiHeadAttention(nn.Module):
    ...
    def _split_heads(self, x):
        ...
        shape = x.shape
        return x.view(shape[0], shape[1], self.num_experts, self.num_heads,
                      shape[2] // (self.num_heads * self.num_experts)).permute(0, 2, 3, 1, 4)
    ...
    def _merge_heads(self, x):
        ...
        shape = x.shape
        return x.permute(0, 3, 1, 2, 4).contiguous().view(shape[0], shape[3], self.num_experts,
                                                          shape[4] * self.num_heads)
...
def gen_embeddings(vocab):
    ...
    elif config.emb_file is not None:
        ...
        if ty == "test":
            ...
            for i, (greedy_sent, beam_sent) in enumerate(zip(sent_g, sent_b)):
                ...
                batch_lst.append([" ".join(s) for s in
                                ...
                                batch['input_txt'][i]))
                rf = " ".join([ele for lis in batch["target_txt"][i] for ele in lis] if config.dataset == "empathetic_dialogues" else batch['target_txt'][i])
