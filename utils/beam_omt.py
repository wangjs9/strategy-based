...
class Translator(object):
    ...
    def beam_search(self, batch, max_dec_step):
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, enc_output, inst_idx_to_position_map, n_bm, mask_src, graph_info=None, strategy_info=None):
            ...
            def predict_word(dec_seq, enc_output, n_active_inst, n_bm, mask_src, graph_info, strategy_info):
                ## masking
                mask_trg = dec_seq.data.eq(config.PAD_idx).unsqueeze(1)
                mask_src = torch.cat([mask_src[0].unsqueeze(0)] * mask_trg.size(0), 0)

                if graph_info != None:
                    cause_repr, concept_label, distance, head, tail, triple_repr, triple_label, vocab_map, map_mask = graph_info
                    ...
                    gate, cpt_probs_vocab = self.model.gcause.comp_pointer(out, concept_label, distance, head, tail,
                                                            triple_repr, triple_label, vocab_map, map_mask)
                    prob = self.model.generator(out) * (1 - gate) + gate * cpt_probs_vocab
                elif config.model == 'strategy':
                    out, attn_dist = self.model.decoder(self.model.embedding(dec_seq), enc_output, (mask_src, mask_trg))
                    out = self.model.cvae.inference(strategy_info.unsqueeze(1).repeat(out.size(0), out.size(1), 1), out)
            ...
            word_prob = predict_word(dec_seq, enc_output, n_active_inst, n_bm, mask_src, graph_info, strategy_info)
        ...
        with torch.no_grad():
            # -- Encode
            enc_batch, _, _, _ = get_input_from_batch(batch)
            strategy_info = batch['strategy']
            ...
            if config.model == 'multihop':
                ...
                if use_graph:
                    ...
                    triple_repr, cause_repr = self.model.gcause.comp_cause(concept_ids, relation, head, tail, triple_label)
                    graph_info = (cause_repr, concept_label, distance, head, tail, triple_repr, triple_label, vocab_map, map_mask)
            ...
            for len_dec_seq in range(1, max_dec_step + 1):
                ...
                active_inst_idx_list = beam_decode_step(inst_dec_beams, len_dec_seq, src_enc,
                                                        inst_idx_to_position_map, n_bm, mask_src, graph_info, strategy_info)
